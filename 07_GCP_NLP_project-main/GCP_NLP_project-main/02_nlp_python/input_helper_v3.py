...
# nargs 應該讀取的命令行參數個數，可以是具體的數字，或者是?號，當不指定值時對於Positional argument 
# 使用 default，對於 Optional argument 使用 const；或者是 * 號，表示 0 或多個參數；或者是 + 號表示 1 或多個參數。  
...
parser.add_argument(
    '-s',
    '--split',
    action='store_true',    # 命令行遇到参数时的动作，默认值是 store。
...
parser.add_argument(
    ...
    '--verbose',
    action='store_true',
    help='Whether to print process details to stderr.')  # 是否将流程详细信息打印到stderr  # stderr标准出错文件
...
MLUSE_UNSPECIFIED = 'UNSPECIFIED'   # MLUSE = machine learning use
MLUSE_TRAIN = 'TRAIN'
...
def _IsGcsPattern(file_pattern):       # 是不是用 gs:// 開頭
...
def PrepareImportFilesFromFilePattern(input_file_pattern, import_files):
  ...
  if _IsGcsPattern(file_pattern):
    ...
    for local_file in glob.glob(normalized_pattern):  # glob 文件名模式匹配，不用遍歷整個目錄判斷每個文件是不是符合。
